    ...
    for subject_folder in subject_folders:
        ...
        if pet and not ct:
            ...
            logging.info(f'Error analysis completed for: {ct_atlas}, segmentation error stats saved in '
                         f'{segmentation_error_stats}')
            print(f"- Error analysis completed for: {ct_atlas}, segmentation error stats saved in"
                  f" {segmentation_error_stats}")
            ...
            spinner.succeed(f"PET/CT alignment completed")
            logging.info('Calculating SUV parameters for SUV extraction...')
            spinner = Halo(text='Calculating SUV image for SUV extraction...', spinner='dots')
            spinner.start()
            ...
            spinner.succeed(f"Calculated SUV image for SUV extraction!")
            if pp.brain_exists(pet_file[0]):
                ...
                logging.info(
                    'SUV values stored in ' + os.path.join(subject_folder, processing_folder + '-SUV-values.csv'))
            logging.info(f'Conducting error analysis in similarity space for: {moose_ct_atlas}')
            print(f'- Conducting error analysis in similarity space for: {moose_ct_atlas}')
            segmentation_error_stats = os.path.join(subject_folder, processing_folder +
                                                    '-Risk-of-segmentation-error.csv')
            ea.similarity_space(moose_ct_atlas, sim_space_dir, segmentation_error_stats)
            logging.info(f"Error analysis completed for: {moose_ct_atlas} and segmentation error stats are stored in: "
                         f"{segmentation_error_stats}")
            print(f"- Error analysis completed for: {moose_ct_atlas} and segmentation error stats are stored in: "
                  f"{segmentation_error_stats}")
